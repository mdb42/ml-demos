...
####################################################################################################
# The Bigram Language Model with Softmax-based Feature Calculation
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        B, T, C = x.shape
        Tril = torch.tril(torch.ones(T, T)).to(x.device)  # Lower triangular matrix of ones
        
        # Mask future tokens by filling with -inf
        W = Tril.masked_fill(Tril == 0, float('-inf'))  # Mask future tokens
        
        # Apply softmax to compute normalized attention weights
        W = F.softmax(W, dim=-1)

        # Perform batched matrix multiplication: (B x T x T) @ (B x T x C) -> (B x T x C)
        X_bag_of_words_3 = W @ x

        # Proceed with generating logits or further processing as needed
        logits = self.token_embedding_table(idx)  # Placeholder for further steps
        return logits
