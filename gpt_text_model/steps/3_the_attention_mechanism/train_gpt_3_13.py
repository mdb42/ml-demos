...
class SelfAttentionHead(nn.Module):
    def __init__(self, head_size):
        ...
        self.query = nn.Linear(C, head_size, bias=False)  # Linear transformation for queries
        self.key = nn.Linear(C, head_size, bias=False)    # Linear transformation for keys
        self.value = nn.Linear(C, head_size, bias=False)  # Linear transformation for values
    ...
    def forward(self, x):
        ...
        # Compute affinities (Q dot K^T)
        affinities = Q @ K.transpose(-2, -1) / (self.head_size ** 0.5)  # Shape (B, T, T)

        # Dynamically create a mask for future tokens
        mask = torch.tril(torch.ones(T, T, device=x.device))  # Shape (T, T)
