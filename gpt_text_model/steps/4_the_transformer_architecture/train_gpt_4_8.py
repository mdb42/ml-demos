...
class TransformerBlock(nn.Module):
    def __init__(self, num_heads, head_size):
        ...
        self.attention_projection = nn.Linear(C, C)  # Projection after self-attention
        self.feedforward_projection = nn.Linear(C, C)  # Projection after feedforward network

    def forward(self, x):
        # Communication through multi-head attention with residual connection and projection
        x = x + self.attention_projection(self.multi_head_attention(x))  # Residual + Projection

        # Computation through feedforward network with residual connection and projection
        x = x + self.feedforward_projection(self.feed_forward(x))  # Residual + Projection
