...
class BigramLanguageModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.token_embedding_table = nn.Embedding(vocab_size, n_embed)  # Embedding into n_embed dimensions
        self.linear_layer = nn.Linear(n_embed, vocab_size)  # Project embeddings back to vocab_size logits
    
    def forward(self, idx, targets=None):
        # Embed the tokens (B x T -> B x T x n_embed)
        x = self.token_embedding_table(idx)  # Shape (B, T, n_embed)
        
        # Project embeddings to logits using the linear layer
        logits = self.linear_layer(x)  # Shape (B, T, vocab_size)
