...
####################################################################################################
# The Bigram Language Model with Vectorized Feature Calculation
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # Vectorized computation using a lower triangular weight matrix for averaging
        B, T, C = x.shape
        W = torch.tril(torch.ones(T, T)).to(x.device)  # Lower triangular matrix of shape (T, T)
        W = W / W.sum(dim=1, keepdim=True)  # Normalize to make sure each row sums to 1 for averaging

        # Perform batched matrix multiplication: (B x T x T) @ (B x T x C) -> (B x T x C)
        X_bag_of_words_2 = W @ x

        # (Optionally: proceed with other operations like generating logits)
        logits = self.token_embedding_table(idx)  # Placeholder for further processing
        return logits
