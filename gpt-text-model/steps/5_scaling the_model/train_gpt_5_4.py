...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads, head_size):
        ...
        self.projection = nn.Linear(num_heads * head_size, C)  # Project back to C dimensions
...
class TransformerBlock(nn.Module):
    def __init__(self, num_heads, head_size, dropout=0.1):
        ...
        self.feed_forward = FeedForwardLayer()  # Feedforward network
        self.ln1 = nn.LayerNorm(C)  # LayerNorm before self-attention
        self.ln2 = nn.LayerNorm(C)  # LayerNorm before feedforward network
        self.dropout = nn.Dropout(dropout)  # Dropout layer

    def forward(self, x):
        # Communication through multi-head attention with LayerNorm and Dropout
        x = x + self.dropout(self.multi_head_attention(self.ln1(x)))  # Apply Dropout after multi-head attention

        # Computation through feedforward network with LayerNorm and Dropout
        x = x + self.dropout(self.feed_forward(self.ln2(x)))  # Apply Dropout after feedforward network
        return x


####################################################################################################
# The Bigram Language Model with Final LayerNorm

class BigramLanguageModel(nn.Module):
    def __init__(self, n_layer, num_heads, head_size):
        super().__init__()
        ...
        # Stack multiple Transformer Blocks dynamically based on n_layer
        self.transformer_blocks = nn.Sequential(
            *[TransformerBlock(num_heads, head_size) for _ in range(n_layer)]  # Stack n_layer blocks
        )
        
        self.final_layer_norm = nn.LayerNorm(C)  # Final LayerNorm before the output
...
model = BigramLanguageModel(n_layer=n_layer, num_heads=num_heads, head_size=head_size)
